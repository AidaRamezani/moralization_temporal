...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='configs', help='Directory to store the config files')
    ...
    assert 'data_features' in config, f"Key 'data_features' not found in config file {args.config_dir}"
    assert 'year' in config['data_features'][args.data], f"Key 'year' not found in config file {args.config_dir}"
    
    year_range = config['data_features'][args.data]['year']
    model_name = config['model_name']
    test_result_path = config['test_results_path']
    data_name = args.data
    train_sections = range(0, 5)
    ...
    word_count = pd.read_csv(os.path.join(config['store_dir'], f'{data_name}_word_count.csv'))
    ...
    ts_df['outputs_z'] = [(o - mean_values[s,p]) / std_values[s, p] for o ,w, y , p,s in
                            zip(ts_df.outputs, ts_df.words, ts_df.year, ts_df.property, ts_df.train_section)]
    ...
    for year in year_range:
        d = pickle.load(open(os.path.join(config['store_dir'], 'sentiments_{}_{}_{}.pkl'.format(data_name, year, model_name)), 'rb'))
        df = pd.DataFrame(d.items(), columns = ['words','sentiments'])
        df['year'] = year
        sentiment_df = pd.concat([sentiment_df, df], ignore_index=True)
    
    sentiment_df.to_csv('data/SWOW_prediction/eval/{}_sentiments.csv'.format(data_name), index=False)
