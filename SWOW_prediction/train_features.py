...
from SWOW_prediction.utils import get_config, get_l1_regularization, get_likelihood_loss
...
def train(config_file):
    ...
    if config_file["baseline"]:
        model = BasicPropertyPredictor(
            in_channels, dropout,
        )  # Baseline BERT based model
    else:
    ...
    for super_epoch in tqdm.tqdm(range(num_super_epochs), desc="Super Epochs"):
        for epoch in tqdm.tqdm(
            range(num_epochs), desc=f"Epochs (Super Epoch {super_epoch})",
        ):
            # Training step
            model.train()
            optimizer.zero_grad()
            ...
            loss = loss_function(output_train, train_target) + get_l1_regularization(
                model, l1_lambda,
            )
            ...
            with torch.no_grad():
                if not config_file["baseline"]:
                    ...
                    output = model(
                        embeddings,
                    )  # Baseline model only words with embeddings, and not the graph

                output_dev = output[dev_index]

                # Calculate metrics
                dev_loss = loss_function(
                    output_dev, dev_target,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path", type=str, default="SWOW_prediction/config_features.yml",
    )
