...
def get_edge_index_from_edges(
        ...
        nodes_total: Optional[Dict[str, float]] = None,
        normalize: bool = False,
        sample_negatives: bool = False,
        negative_sample_num: Optional[int] = None
...
def get_word_count(
    ...
    sentences: List[str], 
    swow_cues: Optional[List[str]] = None,
    ...
    if strategy == 'frequency':
        cues = sorted(word_count.keys(), key=lambda x: -word_count[x])[:k] #We will only select the top k frequent words 
...
def co_occurrence(
    ...
    cues: List[str], 
    two_gram: bool = False, 
    two_grams: Optional[List[str]] = None,
    window_size: int = 1
    ...
    for tokenized_data in articles:
        ...
        if two_gram and two_grams:
            ...
            while i < n:
                ...
                potential_two_gram = f"{token} {tokenized_data[i + 1]}" if i < n - 1 else ""
...
def get_data_graph(
    ...
    def get_pmi(word: str, df: pd.DataFrame, col_totals: Dict[str, int], 
                total: int, cues: List[str], n: int = 25) -> Dict[str, float]:
        """Calculate PMI for a word with other words."""
        words = np.array(df.columns)
        word_occurrence = df.loc[word, :].values
        
        # Calculate PMI
        denominator = np.array([col_totals.get(k, 0) * col_totals.get(word, 0) / total 
                               for k in words])
        
        
        valid_indices = denominator > 0
        word_occurrence_valid = np.zeros_like(word_occurrence, dtype=float)
        word_occurrence_valid[valid_indices] = word_occurrence[valid_indices] / denominator[valid_indices]
        ...
        sorted_occurrences = -np.sort(-word_occurrence[indices])[:min(n, len(indices))] #Selecting top n neighbors
        ...
        return dict(zip(wanted_words, sorted_occurrences))
    ...
    edge_index, edge_weight, vocab_mapping, _ = get_edge_index_from_edges(
        edges, vocab_mapping, fill, add_self_loop, nodes_total, normalize=True, sample_negatives=False
    )
        ...
        for j, token in enumerate(tokenized_sentence):
            if token in cues:
                ...
                try:
                    i_th_positions = get_word_position(
                        token, sentence_encodings, sentences[i], 
                        lemmatized_tokens[i], token_list[i], 
                        special_token_id, tokenizer, model_name=model_name
                    )
            ...
            if j < len(tokenized_sentence) - 1:
                ...
                if two_gram_text in two_grams and two_gram_text in sentences[i]:
                    ...
                    try:
                        i_th_positions = get_word_position(
                            two_gram_text, sentence_encodings, sentences[i], 
                            lemmatized_tokens[i], token_list[i], 
                            special_token_id, tokenizer, model_name=model_name
                        )
                        curr_positions.append(i_th_positions)
                    except Exception as e:
                        print(f"Position not retrieved for two-gram '{two_gram_text}': {e}")
...
def store_encoding_data(
    ...
    **kwargs
    ...
    sentence_positions, all_encodings = get_encodings(
        ...
        cues, two_grams, tokenizer, model_name, max_length
    )
    
    return sentence_positions, all_encodings


def store_embedding_data(model_name, year=2000, max_length=200, data_name='coha', **kwargs):
    """
    ...
    encoding_file = f'data/SWOW_prediction/{data_name}_{year}_encodings_{model_name}.pkl'
    try:
        sentence_positions, all_encodings = pickle.load(open(encoding_file, 'rb'))
    except FileNotFoundError:
        raise FileNotFoundError(f"Encoding file {encoding_file} not found. Run the encoding step first.")
    ...
    embedding_data = {
        ...
        for w in final_cue_embeddings if final_word_counts[w] > 0
    }
...
def store_textual_data(
    ...
    **kwargs
    ...
    try:
        ...
        print(f"Warning: Embedding file not found or corrupted. Processing may be incomplete.")
        embedding_data = {}
...
def save_word_count(data_name: str = 'coha', model_name: str = 'bert-base-uncased',storing_dir = './data/SWOW_prediction') -> None:
    """
    ...
    for year in year_range:
        try:
            final_cue_embeddings, final_word_counts, _ = pickle.load(
                open( os.path.join(storing_dir, f'{year}_{data_name}_emb_{model_name}.pkl'), 'rb')
            )
    ...
    word_count_df.to_csv(os.path.join(storing_dir, f'{data_name}_word_count.csv'), index=False)
...
def construct_property_dataset(
    ...
    **kwargs
    ...
    if feature not in ['previous_link', 'polarity']:
        raise ValueError("Feature must be either 'previous_link' (moral relevance) or 'polarity' (moral polarity)")
    ...
    stored_data_dir = (f'property_{feature}_{data_name_store}_{year}_{model_name}_'
    ...
    try:
        ...
        raise FileNotFoundError(f"Moral foundations data file not found at data/SWOWEN/moralized_v{swow_version}_mfd.csv")
    ...
    if feature == 'previous_link':  # Moral relevance
        ...
        if take_log:
            property_data['previous_link'] = np.log(property_data['previous_link'] * 100 + 1)
        ...
        property_data = property_data.loc[property_data['polarity'] != 0.5]  # pos != neg
    ...
    edge_indices = [
        k for k in range(text_edge_index.shape[1]) 
        if (vocab_mapping_reverse.get(int(text_edge_index[0, k])) in cues and
            vocab_mapping_reverse.get(int(text_edge_index[1, k])) in cues)
    ]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Process textual data for word embeddings and graph construction")
    parser.add_argument('--year', type=int, default=2000, help="Year to process")
    parser.add_argument('--model', type=str, default='bert-base-uncased', help="Model to use")
    ...
    parser.add_argument('--length', type=int, default=200, help="Maximum sequence length")
    parser.add_argument('--node_neighbors', type=int, default=100, help="Number of neighbors for graph construction")
    parser.add_argument('--data_path', type=str, default='./data/COHA.zip', help="Path to data file")
    ...
    if not os.path.exists(args.data_path):
        raise argparse.ArgumentTypeError(f"Data path: {args.data_path} is not a valid path")
    ...
    if args.function == 'encoding':
        ...
        output_path = os.path.join(args.store_dir, f'{args.data}_{args.year}_encodings_{args.model}.pkl')
        ...
        output_path = os.path.join(args.store_dir, f'{args.year}_{args.data}_emb_{args.model}.pkl')
        with open(output_path, 'wb') as f:
        ...
        output_path = os.path.join(args.store_dir, f'sentiments_{args.data}_{args.year}_{args.model}.pkl')
        with open(output_path, 'wb') as f:
