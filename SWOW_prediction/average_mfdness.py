...
def get_neighbors_for_words(word_list, year, freqs) -> Dict[str, pd.DataFrame]:
    ...
    for (src, tgt), weight in zip(relevant_edges, relevant_weights, strict=False):
        src_item = src.item()
        tgt_item = tgt.item()
        ...
        if src_item in word_indices:
            neighbors_dict[vocab_mapping_reverse[src_item]].append(
                {"word": vocab_mapping_reverse[tgt_item], "weight": weight.item()},
            )
            ...
            neighbors_dict[vocab_mapping_reverse[tgt_item]].append(
                {
                    ...
                    "weight": (
                        weight.item() if isinstance(weight, np.ndarray) else weight
                    ),
                },
            )
...
def add_mfdness(data, year_neighbors, word_year_df):
    ...
    def assign_mfdness(word, year):
        ...
        neighbor_df["mfdness"] = neighbor_df["word"].apply(
            lambda x: 1 if x in mfd_words else 0,
        )
        ...
        average_mfdness = (
            np.sum(neighbor_df["mfdness"] * neighbor_df["weight"]) / sum_weights
        )
...
def main(data) -> None:
    ...
    if data == "coha":
        ...
        if not os.path.exists(ts_path):
            raise FileNotFoundError("The specified time series file {} does not exist.".format(ts_path))
    ...
    year_freqs = {}
    for _year_index, year in enumerate(years):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Calculate moral foundation scores for words across years",
    )
    parser.add_argument(
        "--data", type=str, default="coha", help="Dataset name (default: coha)",
    )
